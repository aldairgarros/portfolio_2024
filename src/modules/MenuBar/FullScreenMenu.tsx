      ...
      <div className="flex flex-col gap-6 w-full h-full p-8 pt-12">
        <div className="flex flex-col gap-2">
          {links.map((link, ind) => (
            <Link
              key={link.hash}
              className={`text-2xl font-medium font-heading text-primary-900 dark:text-primary-50 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all duration-300 cursor-pointer focus:ring-2 focus:ring-emerald-400 focus:outline-none rounded ${
        ...
        <div className="flex gap-2 mt-4">
          {(["en", "br"] as const).map((lang, ind) => (
            <button
              key={lang}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 cursor-pointer focus:ring-2 focus:ring-emerald-400 focus:outline-none ${
                open ? "translate-x-0 opacity-100" : "-translate-x-4 opacity-0"
              } ${
